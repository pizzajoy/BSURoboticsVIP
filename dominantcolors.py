...
def rgba2rgb(rgba):
    ...
    for ichan in range(channels.shape[-1]):
        w = alpha / 255.
        out[..., ichan] = np.clip(w * channels[..., ichan] + (1 - w) * background[ichan],
                                  a_min=0,
                                  a_max=255)
    out.astype(np.uint8)
    return out
...
def find_dominant_colors(img_colors, count):
    ...
    dominant_colors = get_dominant_colors(root)
    quantized_image = get_quantized_image(img_colors, classes, dominant_colors)
    return dominant_colors, quantized_image
...
def get_dominant_colors(root):
    ...
    while len(queue):
        ...
        color[0] = curr_node.mean[0] * 179
        color = np.clip(color, 0, 255)
        color[0] = np.clip(color[0], 0, 179)
        color = color.astype(np.uint8)
        dominant_colors.append([curr_node.class_id, curr_node.num_pixel, color.tolist()])
...
def get_dominant_colors_for(image, num_colors):
    ...
    return find_dominant_colors(image, num_colors)


def get_quantized_image(img, classes, dominant_colors):
    classes = classes.reshape(img.shape[0:2])
    ret = img.copy()
    for color in dominant_colors:
        curr_class = color[0]
        curr_color = color[2]
        idx = (classes == curr_class).nonzero()
        ret[idx[0], idx[1], :] = curr_color
